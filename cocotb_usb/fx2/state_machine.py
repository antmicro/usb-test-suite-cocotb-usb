...
class StateMachine(dict):
    ...
    Initialize the StateMachine like a dictionary (pass a dictionary as second argument,
    or use kwargs as in dict(a=..., b=...)).
    """
    ...
    def __setitem__(self, key, value):
        if not isinstance(key, self._states_type):
            raise KeyError('Key should be a subclass of %s: %s' % (self._states_type, key))
